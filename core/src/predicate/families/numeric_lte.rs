//! `numeric_lte(attr, n)` — true when the entity declares numeric attribute `attr`
//! with a value ≤ `n`.
...
const FAMILY: &str = "numeric_lte";

#[derive(Debug)]
pub struct NumericLte {
    id: String,
    attr: String,
    threshold: f64,
    labels: StoredLabels,
...
mod tests {
    ...
    #[test]
    fn matches_when_value_below_or_equal() {
        let p = NumericLte::new("arrondissement".into(), 5.0, labels());
        assert!(p.matches(&ent(1.0)));
        assert!(p.matches(&ent(5.0)));
    }

    #[test]
    fn rejects_when_value_above() {
        let p = NumericLte::new("arrondissement".into(), 5.0, labels());
